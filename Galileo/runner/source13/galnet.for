...
C PROGRAM TO TRANSPOSE A MATRIX
  ...
  300 write(6,4)
    ...
    8 format(a1)
      ...
      hfile='\galileo\help\real.hlp'
      call assist(noy,hfile,*300)


      if(noy.eq.'y'.or.noy.eq.'Y')iflag=1
...
c      write(6,6)ncon,natt   !  debug **
    ...
    9 write(6,7)
    7 format('$How many dimensions do you wish?')
      read(5,*,err=52)ndim
        go to 50

   52 hfile='\galileo\help\ndim.hlp'
      call assist('?',hfile,*9)


  50  print*, ' Is it a CENTROID SCALAR PRODUCTS MATRIX?'
      print*, ' (Unless you''re sure it is, it probably isn''t)'
      read(5,8)noy

      hfile='\galileo\help\scpin.hlp'
      call assist(noy,hfile,*50)
...
c -- end of engorgement
   60 print*, ' And where would you like the output, Wily One?'
      ...
      hfile='\galileo\help\crdsout.hlp'
