...
C       stop
  ...
  314 format(' These can''t be the labels; there''s a problem.'/
     +' Since I''m only a demo program, I''m going to die ugly.')
	stop
c
c   31 print 315
c  315 format(' Where are the labels?')
c      call prompt
c      read(5,122,err=26)ifile
c
c      hfile='or_lbl.hlp'
c      call assist(ifile(1:1),hfile,*31)
   31 ifile='c:\listiac\file.lbl'
...
C      if(lbl)go to 112
   ...
   25 PRINT 316
  316 format(' Sorry, Lunimosity, there are no data here.'
     +'  I''m croaking.')
	 stop

c   30 print 317
c 317  format(' Where are the data?')
c      call prompt
   30 ifile='c:\listiac\file.win'
c      read(5,122,err=25) ifile
...
c now get the external inputs to the system
   ...
   7  do 88 i=1,ncon
      print 322
 322  format(' Enter as many items as you want, Your Uselessness.
     + -1 when done)')
C      call prompt
      read(5,9,end=12)ANS
...
c      read(5,*,err=11)ext(Jj)
   ...
   12 print 324
  324 format(' And the winners are...',//)
...
c__________________A simple linear function_________________________________
   ...
   18  format(20X,a30,' Activation level = ',f6.4)
       if(pstop.eq.20)then
       pstop=1
       print*,' ...press enter for more...'
       read(5,*)
...
c     for dos
      ...
      ifile='galileo/help/'//hfile(1:len)
