...
C PROGRAM GALILEO
c ***  Version 5.6  June,1993 ***
c Copyright 1974 - 2015
c Joseph Woelfel
...
C RESERVE STORAGE
      ...
      INTEGER BLNK,ESS,PLT,PLTONL
      INTEGER EIG,ROT,RD
      CHARACTER * 40 RUN
      character*80 ifile,rsfile    ! 1/Feb/1988 - jw
      ...
      DATA BLNK/' '/
      DATA ESS/'s'/
      DATA NAMES/'Joe ','Woel','fel',' ',' ',
     +'Rick',' Hol','mes',' ',' '/
c      DATA NCNT/2/
c
c              MAIN PARAMETERS
c
      call datwrt('GALILEO_PC','V2.0')
      write(5,400)
 400  format(' Where is your runstream?')
  ...
  222 format(a80)
   ...
   10 FORMAT(15X,A40)
      ...
      WRITE(LFNO,405) RUN
  405 FORMAT (30X,' GALILEO Version 5.6 Control List '//////////31X,
     +'Run Name is...................',A40)
...
C PRINT THE HEADER
      ...
      call prntcon(12,lfno)
       WRITE(LFNO,1000) NAMES
1000  FORMAT(/10X,'GALILEO Version 5.6 ** Update June, 1993 **',///
     +10X,' Copyright 1977 - 1993',//
     +(10X,5A4))
      write(lfno,23)
  23  format(10x,'Scott Danielsen'/10x,'Rudolf Zelf')

      WRITE(LFNO,1001) RUN,MSIZE,NSETS,ICHR
1001  FORMAT(/10X,'This program is the exclusive '/
     +10X,'property of The Galileo Company               '/
     +,10X,'23A Durham Drive                '/
     +10X,'Amherst, NY 14228                       '/
     +10X,'Use of this program without written '/
     +10X,'authorization from The Galileo Company  '/
     +10X,'is strictly prohibited. '///
     +1X,'GALILEO  Version 5.6 June, 1993'//1x,'GALILEO is a trademark
     +of The Galileo Co., 23A Durham Dr., Amherst, NY,14228
     +',///
...
C
      WRITE(LFNO,1004) RUN
 1004 FORMAT ('2'//////50X,'End Program GALILEO'//46X,A40,5X,'PAU')
c
...
3     CONTINUE
      WRITE(LFNO,720)
  720  FORMAT('0**********ILLEGAL OPERATION**********')
      GO TO 103
C
...
C RZ 10/85 CHARACTER
      CHARACTER*1 IN(76)
      CHARACTER*1 ZERO,BLNK,MINUS !NINE
...
4109  FORMAT('0 Cell with maximum distance is',2I3
...
9     CONTINUE
      AVEERR=TVAR/MNC
      PRINT*, ' Ave std err is ',aveerr
      tvar=0	! a bug fix 6/27/93 jw
C
      WRITE(LFNO,77) AVOBV
77    FORMAT('0 Average observations per cell ',F10.4)
...
C      IMPLICIT REAL (A-H,O-Z)
      ...
      CHARACTER*40 LABELS(40)
      real best2(10)
      common/besmes/best2,labls2,rleft,cons
      DATA BEST2 /10*100./
...
C
      SUBROUTINE XERROR
      common/besmes/best2,labls2,rleft
      COMMON/STATI/AVEERR,MNC
      character*11 labls2(10)
...
410   FORMAT('0     MESSAGE EFFECTIVENESS...FOR THE MESSAGE',4I3/
...
C
C
C     GALILEO IS A PROGRAM FOR METRIC MULTIDIMENSIONAL SCALING
C     USING THE METHOD OF YOUNG AND HOUSEHOLDER(1938) AS MODIFIED
C     BY TORGERSON(1958).  IT IS DESIGNED FOR OVER-TIME ANALYSIS
C     OF ANY NUMBER OF DATASETS.  IT INCLUDES PROCEDURES FOR DESIGNING
C     OPTIMAL PERSUASIVE MESSAGES FOLLOWING WOELFEL ET AL.(1977).
C
      IMPLICIT REAL (A-H,O-Z)
      COMMON/AMGCOM/ST,TG,AMG
      COMMON/AXIS/AX(3)
      COMMON/BASE/MNS,EIG,ROT,RD
      COMMON/CASES/CASES
      COMMON/EXVAL/EXVAL
      ...
      COMMON/NOADR/NOADR
      COMMON/OPT/OPT
      COMMON/PARM/BOXD,SIZE,MWAVE,TYPE,COLOR,VIEW(4),NAME(2)
      COMMON/PLT/PLT
      COMMON/PLTONL/PLTONL
      COMMON/RAWDAT/IDLEN,PAIRS,FORM
      COMMON/RUN/RUN
...
C
      CHARACTER * 40 RUN
      INTEGER AX,PAIRS,UNMASS
      CHARACTER*20 FORM
      CHARACTER*1 CHK(8)
      INTEGER EIG,ROT,RD
      INTEGER CASES,AMG(4),ST,TG,TRF,QUE,XCOR
      INTEGER STATIC,DYNAM,PLT,PLTONL
      INTEGER TYPE,COLOR,VIEW
      INTEGER OPT(30)
C
      REAL MAXVAL
C
      ...
      DATA EXVAL/1000./
      DATA HGTLET/0.42/
      DATA IDLEN,PAIRS/8,8/
      DATA FORM /'(8A1,8(2I2,F5.0))'/
C RZ NOVEMBER 23,1984 CHANGE PUCH UNIT TO 9
      DATA LFNI,LFNO,LFNP/5,6,9/
      DATA LOGIC /0/
      DATA MAINV/0/
      DATA MIDDLE /0/
      DATA MAXITR /500/
...
c
      ...
      year = year-1900
      write(6,100)prog,vers,month,day,year,hour,minute,second
